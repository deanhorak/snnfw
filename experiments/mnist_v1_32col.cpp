/**
 * @file mnist_v1_32col.cpp
 * @brief MNIST classification using 32-column optimized V1 architecture
 *
 * Architecture (32 columns - optimized for accuracy):
 * - 16 orientation columns (8 orientations × 2 frequencies)
 * - 6 center-surround columns (3 scales × 2 types)
 * - 10 specialized detector columns:
 *   * 4 top-loop detectors (for 4→9, 7→9)
 *   * 4 gap detectors (for 4→9)
 ...
 *
 * Target: ~14K neurons, ~1.1M synapses (2x 16-col, 36% less than 50-col)
 * Note: Removed bottom-curve detector to avoid digit 7 false positives
 */
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // 32-COLUMN Architecture (optimized for accuracy):
        // - 8 orientations × 2 spatial frequencies (low + high) = 16 columns
        ...
        //   * 4 gap detectors (for 4→9)
        //   * 2 horizontal-bar detectors (for 3→5)
        // Total: 32 columns (2x optimized, 36% less than baseline)
        // Target: ~14K neurons, ~1.1M synapses (optimized approach)
        // Note: Removed bottom-curve detector to avoid digit 7 false positives
        const int NUM_ORIENTATIONS = 8;  // 0°, 22.5°, 45°, 67.5°, 90°, 112.5°, 135°, 157.5°
        ...
        const int NUM_SPECIALIZED = 10;  // Top-loop (4), gap (4), horizontal-bar (2)
        ...
        // Create specialized detector columns
        // 4 top-loop + 4 gap + 2 horizontal-bar = 10 columns
        std::cout << "\n=== Creating Specialized Detector Columns ===" << std::endl;

        // Top-loop detectors (4 columns with different sensitivities)
        for (int i = 0; i < 4; ++i) {
            CorticalColumn col;
            col.orientation = 0.0;
            ...
            if (i == 1) {
                for (auto& row : col.gaborKernel) {
                    for (auto& val : row) {
                        val *= 1.3;  // Slightly more sensitive
                    }
                }
            } else if (i == 2) {
                for (auto& row : col.gaborKernel) {
                ...
                for (auto& row : col.gaborKernel) {
                    for (auto& val : row) {
                        val *= 1.8;  // Most sensitive version
                    }
                }
            }
        ...
        // Gap detectors (4 columns with different sensitivities)
        for (int i = 0; i < 4; ++i) {
            CorticalColumn col;
            col.orientation = 0.0;
            col.spatialFrequency = 0.0;
        ...
        std::cout << "\n✓ Created " << NUM_COLUMNS << " cortical columns (32-COLUMN OPTIMIZED):" << std::endl;
        std::cout << "  - " << NUM_ORIENTATION_COLUMNS << " orientation columns (8 orientations × 2 frequencies)" << std::endl;
        ...
        std::cout << "    * 4 top-loop detectors (4→9, 7→9)" << std::endl;
        std::cout << "    * 4 gap detectors (4→9)" << std::endl;
