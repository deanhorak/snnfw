...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // OPTIMIZED Architecture (reduced from 50 to 16 columns for speed):
        // - 4 orientations × 2 spatial frequencies (low + high) = 8 columns
        // - 2 center-surround scales × 2 types (ON-center + OFF-center) = 4 columns
        // - 0 blob detectors (removed for speed)
        // - 4 specialized detectors:
        //   * 2 top-loop detectors (for 4→9, 7→9)
        //   * 2 gap detectors (for 4→9)
        // Total: 16 columns (optimized for speed while maintaining accuracy)
        // Target: ~7K neurons, ~500K synapses (7x reduction from baseline)
        const int NUM_ORIENTATIONS = 4;  // Reduced from 12 (0°, 45°, 90°, 135°)
        const int NUM_FREQUENCIES = 2;   // Low + High
        const int NUM_CS_SCALES = 2;     // Reduced from 4 (small + medium)
        ...
        const int NUM_SPECIALIZED = 4;   // Reduced from 10 (keep most important)
        ...
        // Center-surround parameters (for loop/hole detection) - OPTIMIZED
        // Reduced from 4 scales to 2 for speed
        ...
        const std::vector<std::string> CS_SCALE_NAMES = {"small", "medium"};
        ...
        auto cacheImageBatch = [&](size_t startImg, size_t endImg) {
            for (size_t imgIdx = startImg; imgIdx < endImg; ++imgIdx) {
                const auto& mnistImg = testLoader.getImage(imgIdx);
                gaborCache[imgIdx].resize(NUM_COLUMNS);

                for (int colIdx = 0; colIdx < NUM_COLUMNS; ++colIdx) {
                    auto& col = corticalColumns[colIdx];
                    gaborCache[imgIdx][colIdx] = applyGaborFilter(mnistImg.pixels, col.gaborKernel, LAYER4_SIZE);
                }
            }
        };
